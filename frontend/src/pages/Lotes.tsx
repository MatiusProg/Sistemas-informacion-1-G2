...
export default function Lotes() {
  ...
  const cargarLotes = async () => {
    ...
    try {
      ...
      console.log('[Lotes] cargarLotes token length', token ? token.length : 'missing');
      console.log('[Lotes] API_URL', API_URL);
      if (!token) {
        console.error('[Lotes] access_token no encontrado en localStorage');
        return;
      }
      ...
      if (!response.ok) {
        console.error('[Lotes] cargarLotes error', response.status, data);
        setLotes([]);
        return;
      }
  ...
  const crearLote = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      console.log('[Lotes] token length', token ? token.length : 'missing');
      if (!token) {
        alert('Sesión expirada o token no disponible. Vuelve a iniciar sesión.');
        console.error('[Lotes] access_token ausente al crear lote');
        return;
      }
      const response = await fetch(`${API_URL}/lotes/`, {
        method: 'POST',
  ...
  const eliminarLote = async (id: number) => {
    ...
    try {
      ...
      if (!token) {
        alert('Sesión expirada o token no disponible. Vuelve a iniciar sesión.');
        console.error('[Lotes] access_token ausente al eliminar lote');
        return;
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-soft">
        ...
        <main className="container py-8 max-w-6xl">
          <div className="p-8 text-center bg-white rounded-3xl shadow-card">Cargando lotes...</div>
        </main>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-soft">
      <AppHeader />
      <main className="container py-8 max-w-6xl">
        <div className="p-6 bg-white rounded-3xl shadow-card">
